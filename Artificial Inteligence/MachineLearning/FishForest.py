...
if __name__=="__main__":
    ...
    train_set=data[:int(0.85*len(data))]
    test_set= data[int(0.85 * len(data)):]
    ...
    num=int(input())
    criterion=input()
    classifier=RandomForestClassifier(n_estimators=num,criterion=criterion,random_state=0)
    ...
    for s in range(len(line)):
        if s!=col_index:
            newl.append(line[s])
    print(classifier.predict([newl])[0])
    probabilities=classifier.predict_proba([newl])[0]
    print(probabilities)
